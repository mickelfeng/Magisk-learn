...
// 将skip_initramfs修改为want_initramfs
// 在init\initramfs.c的populate_rootfs，会默认加载boot分区自带的ramdisk（recovery）
// 但如果 do_skip_initramfs 被设置为1，则会调用default_rootfs生成一个极小的rootfs，并在其中创建出/dev/console文件,除此之外没有其他文件。正因如此，当设备正常启动到系统时读取/proc/cmdline，其中会出现skip_initramfs 的参数，但是启动到recovery时不会有此参数。
