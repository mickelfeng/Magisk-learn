...
public:
    ...
    void start() override {
        // 取消挂载 init，这里的init实际上是/data/init,而/data/init 就是原来系统的init
        bool is_rootfs = prepare();
        if (is_rootfs)
            patch_rw_root();
        else
            // 走这里
            // 进行一些初始化任务，主要是在系统启动时对文件系统进行修改和挂载，以便 Magisk 可以正确地运行和隐藏自己。它涉及的操作包括创建临时目录、加载 overlay 文件、修改 init.rc 文件、提取 Magisk 文件、修改 sepolicy 文件等。
            // 
            patch_ro_root();
        exec_init();
    }
...
public:
    ...
    void start() override {
        prepare_data();
        // 这个函数好像没什么用，只是为了执行 first_stage_prep
