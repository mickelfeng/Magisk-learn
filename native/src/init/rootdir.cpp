...
void MagiskInit::patch_ro_root() {
    ...
    if (access(NEW_INITRC, F_OK) == 0) {
        ...
        //修改init.rc,目的是注入magisk服务
        // 它使用 file_readline 函数逐行读取源文件 init.rc，并根据不同的匹配规则进行不同的修改，然后将修改后的内容写入目标文件 。除了修改 init.rc 文件外，这段代码还向目标文件 dest 中注入了一些自定义的 rc 脚本，这些脚本用于执行一些 Magisk 相关的操作，如在 post-fs-data 阶段启动 Magisk、在系统启动完成后启动 Magisk 等。当然如果在目录下寻找init.rc是找不到的，因为等待注入完成后magisk就会删除rootdir目录以及下面的东西，防止被检测到。如果想要保留就需要native/src/core/daemon.cpp 中patchrm_rf((MAGISKTMP + "/" ROOTOVL).data());
    ...
    // Mount rootdir
    //结合日志分析,这里实际上就是mount了修改好的init.rc到原系统init.rc
    magic_mount(ROOTOVL);
