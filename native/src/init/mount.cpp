...
// mount_system_root 尝试挂载系统根目录并且通过apex判断是否进行第二阶段的init,通过日志分析可以发现is_two_stage: [1] ，从这里开始进入第二阶段init
...
void BaseInit::exec_init() {
    ...
    //调用原系统init,参数和之前的一样，也就是 selinux_setup 
    execv("/init", argv);
    // 从这里开始，magisk 正式退出，进入原系统init
    exit(1);
}

// 创建data目录并且将magisk挂载到data
// 然后将关键文件和文件夹复制到/data目录下
void BaseInit::prepare_data() {
    LOGD("Setup data tmp\n");
    xmkdir("/data", 0755);
    xmount("magisk", "/data", "tmpfs", 0, "mode=755");

    //把/init(magiskinit) copy到/data/magiskinit
