...
static bool magisk_env() {
    ...
    for (auto alt : alt_bin) {
        ...
        if (lstat(alt, &st) == 0) {
            ...
            rm_rf(DATABIN);
            //这个alt应该是/data/user_de/0/com.topjohnwu.magisk/install,然后把该目录删除
            cp_afc(alt, DATABIN);
            rm_rf(alt);
            break;
            //这一段循环其实是把/data/user_de/0/com.topjohnwu.magisk/install里的文件cp到/data/adb/magisk里
        }
    }
    rm_rf("/cache/data_adb");

    // Directories in /data/adb
    //在/data/adb目录下创建下面几个目录
    xmkdir(DATABIN, 0755);
    xmkdir(SECURE_DIR "/post-fs-data.d", 0755);
    xmkdir(SECURE_DIR "/service.d", 0755);
    restorecon();

    //这里的busybox是从/data/user_de/0/com.topjohnwu.magisk/install里cp过来的
    if (access(DATABIN "/busybox", X_OK))
        return false;

    sprintf(buf, "%s/" BBPATH "/busybox", MAGISKTMP.data());
    mkdir(dirname(buf), 0755);
     //把busybox复制到/dev/xxx/.magisk
    cp_afc(DATABIN "/busybox", buf);
    exec_command_async(buf, "--install", "-s", dirname(buf));

    if (access(DATABIN "/magiskpolicy", X_OK) == 0) {
        sprintf(buf, "%s/magiskpolicy", MAGISKTMP.data());
        //把magiskpolicy复制到/dev/xxx/.magisk
...
static void post_fs_data() {
    ...
    //猜测是解锁/dev/block下的设备,让它们可读可写
    unlock_blocks();
    //挂载到magisk mirrors里,可以结合日志分析
    mount_mirrors();
    //从sqlite数据库查出所有的已授权root的app,然后根据是否卸载app来决定移除授权,因为sqlite记录的是app uid.
    prune_su_access();
    ...
    if (get_prop("persist.sys.safemode", true) == "1" ||
        ...
        //这里应该是判断安全模式,拒绝加载所有模块
        boot_state |= FLAG_SAFE_MODE;
        // Disable all modules and denylist so next boot will be clean
        disable_modules();
        disable_deny();
    } else {
        //执行/data/adb/post-fs-data.d目录下的脚本
...
static void late_start() {
    ...
    // 执行 common 和 module 的 service 脚本.
