...
// 打开原先的 app_process[32|64] 文件，fd 保存到 app_process_[32|64] 中
// 把 magisk 自己的可执行文件 magisk[32|64] (mbin) 复制到 zygisk 目录下的 app_process[32|64] (zbin) ，此处用了 sendfile 直接在内核中复制文件。
// 把 zygisk 目录下假的 app_process (实际是 magisk) bind mount 到原先的 /system/bin/app_process[32|64]
