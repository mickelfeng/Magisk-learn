...
# 第一次将skip_initramfs修改为want_initramfs
# 这里修改的目的是因为三星部分机型不支持 SAR 但是 cmdline 里却有这个参数，正常启动的时候内核会直接忽略，但是 Magisk 会因为有这个参数而判断这台设备是 Legacy SAR，进入 Legacy SAR 流程导致手机循环启动。这里 Patch 的本质是不让 magisk 自己看到错误的参数 https://github.com/topjohnwu/Magisk/pull/4788
...
if [ -f kernel ]; then
  ...
  # 因为如果设置了skip_initramfs代表着Method B启动方式中进入到正常系统而不是recovery，magisk也就无法启动了，修改为want_initramfs屏蔽掉skip_initramfs让系统可以从恢复模式中的ramdisk中启动
